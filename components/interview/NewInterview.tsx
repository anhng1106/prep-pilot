...
export default function NewInterview() {
  ...
  const { handleSubmit, loading } = useGenericSubmitHandler(async (data) => {
    ...
    if (res?.error) {
      return toast.error(res?.error?.message || "Interview creation failed");
    }
  ...
  const handleIndustryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedIndustry = e.target.value as keyof typeof industryTopics;

    setSelectedIndustry(selectedIndustry);
    setTopics(industryTopics[selectedIndustry] || []);
  };
  return (
    <div className="p-4">
